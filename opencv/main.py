...
class centernet():
    def __init__(self, model_path, prob_threshold=0.4, iou_threshold=0.5):
        ...
        self.input_height = int(model_path.split('_')[-1].replace('.onnx', ''))
        self.input_width = int(model_path.split('_')[-1].replace('.onnx', ''))
        self.stride = 4
        self.grid = self._make_grid(int(self.input_width/self.stride), int(self.input_width/self.stride))
    ...
    def detect(self, srcimg):
        img = cv2.resize(cv2.cvtColor(srcimg, cv2.COLOR_BGR2RGB), (self.input_width, self.input_height), interpolation=cv2.INTER_LINEAR)
        ...
        for i in range(cx_cy.shape[0]):
            x = max(0, int((cx_cy[i, 0] - 0.5 * wh[i, 0]) * self.stride * scale_w))
            y = max(0, int((cx_cy[i, 1] - 0.5 * wh[i, 1]) * self.stride * scale_h))
            width = min(srcimg.shape[1] - 1, int(wh[i, 0] * self.stride * scale_w))
            height = min(srcimg.shape[0] - 1, int(wh[i, 1] * self.stride * scale_h))
